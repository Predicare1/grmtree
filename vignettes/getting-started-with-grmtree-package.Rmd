...
The data used for this demonstration were from the Alberta Provincial Project for Outcome Assessment in Coronary Heart Disease (APPROACH) registry, a population-based database of all adults who received cardiac catheterization in the province of Alberta, Canada who completed the eight items of the emotional domain of the MOS-SS two weeks after receiving cardiac catheterization.

**Note:** The dataset used for this walkthrough contains some patient information from the Alberta Provincial Project for Outcome Assessment in Coronary Heart Disease (APPROACH) registry and cannot be shared publicly. To be used in this walkthrough, the data has been de-identified with no personal identifier that can be traced to any patient.
...
mos_grmtree_bon <- grmtree(resp ~ sex + age + bmi + Education + depressed +
                       residency + job + multimorbidity + smoker,  
                       data = resp.data,
                       control = grmtree.control(minbucket = 400,
                                                 p_adjust = "bonferroni"))
```
...
plot(mos_grmtree)
```
More than one subgroup (terminal node) was identified, suggesting the presence of sample heterogeneity with respect to DIF on the MOS-SS emotional domain items. Specifically, the GRMTree identified four distinct subgroups defined by interactions among age, sex and smoking status as splitting variables. Subgroup 1 consists of patients who are ≤62 years old, subgroup 2 consists of females who are > 62 years, subgroup 3 consists of non-smoking males who are > 62 years, and subgroup 4 consists of smoking male patients who are > 62 yearsIn the figure above, the larger the sections of the bars for each response category, the more likely patients are to endorse that category. For a more detailed example on the interpretation of these types of plot, see the article below.
...
**Note:** There were missing values on some covariates. The missing values on these covariates were less than 10%, and included (n, %) sex (9, 0.1%), education (227, 4.0%), and employment status (98, 1.7%), driving status (21, 1.3%), and education level (21, 1.3%). Missing values were not imputed because they were handled by the GRMTree automatically during model fitting by removing only the missing observations from the covariates. The model removed 316 missing values from patients’ characteristics to generate the four subgroups. 
